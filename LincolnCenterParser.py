...
from parser_common_code import \
    initialize_csv_dict, set_start_end_fields_from_start_dt, set_tags_from_dict
...
venue_translations = {
    ...
    'The New York Public Library for the Performing Arts, Dorothy and Lewis B. Cullman Center, Vincent Astor Gallery':
        'New York Public Library for the Performing Arts',
    'The New York Public Library for the Performing Arts, Dorothy and Lewis B. Cullman Center, Plaza Corridor Gallery':
        'New York Public Library for the Performing Arts',
...
class LincolnCenterParser(EventParser):
    ...
    def parse_soup_to_event(self, url, soup):
        ...
        # Program
        program_lines = listing_section('h2', 'eventDetailsReskin22__wrapper--programs-title',
                                        'p', 'eventDetailsReskin22__wrapper--programs-details')
        if program_lines:
            # Program heading
            program_lines = ['<strong>Program</strong>'] + program_lines

        # Performers
        performer_lines = listing_section('h3', 'artist__title',
                                          'p', 'artist__position')
        if performer_lines:
            # performer heading
        ...
        # Price
        try:
            price = soup.find_all('span', attrs={'class': 'ticket-time-price'})[-1].contents[0]\
                .strip()\
                .replace('$','')\
                .replace(' ', '')\
                .replace('–', '-')
