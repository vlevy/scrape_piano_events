...
class ZincParser(EventParser):
    ...
    def parse_soup_to_event(self, url, soup):
        ...
        date_str = str(soup.find('div', attrs={'class': 'offbeat-event-date'}).contents[3].contents[0])
        ...
        # Event description
        paragraphs = soup.find('div', attrs={'class': 'offbeat-event-content'}).find_all('p')
        for p in paragraphs:
            p.attrs = {}
        ...
        # Price
        print("Fill in price from description")

        # Tags
        set_tags_from_dict(csv_dict)

        # Image URL
        image_url = soup.find('div', attrs={'class': 'offbeat-event-featured-image'}).contents[1]['src'].split('?')[0]
