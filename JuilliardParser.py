...
class JuilliardParser(EventParser):
    ...
    def parse_image_url(self, soup) -> str:
        try:
            ...
            # Parses to ('image', 'png', 'eYF57vEs')
            stem, ext, token = re.search(
                "([a-zA-Z0-9_]+)\.(JPG|JPEG|PNG|jpg|jpeg|png)\?itok=(.{8})", image_url
            ).groups()
    ...
    def parse_soup_to_event(self, url, soup):
        ...
        # -----------------------------------
        relevant = (
            is_relevant_to_site(event_name_from_page)
            or is_relevant_to_site_as_accompanied(event_name_from_page)
        ...
        if program_list_element:
            ...
            relevant = (
                relevant
                or is_relevant_to_site(str(program_list))
                or is_relevant_to_site_as_accompanied(str(program_list))
            )
        ...
        if student and instrument:
            ...
            is_precollege
            and is_relevant_to_site(event_name_from_page)
            and "pre-college faculty" not in event_name_from_page.lower()
            and program_list_element
            and program_list_element.find_all("p")
        ):
            # Pre-college recital
            performers = []
