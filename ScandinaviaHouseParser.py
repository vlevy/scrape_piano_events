...
class ScandinaviaHouseParser(EventParser):
    def parse_soup_to_event(self, url, soup):
        ...
        # Event name
        event_name_parts = soup.find_all("h1", attrs={"class": "entry-title"})[
            0
        ].contents
        ...
        # Date: 'Thu—5-2-2019'
        date_str = str(
            soup.find_all("div", attrs={"class": "slide-event--large_date"})[
                0
            ].contents[0]
        )
        ...
        try:
            ...
            logger.info(
                f"Warning: Unable to determine starting time from  date: {repr(start_str)}"
            )
            return None
        else:
        ...
        paragraphs = [
            str(p)
            for p in soup.find("div", attrs={"class": ("event-content")})
            .contents[1]
            .find_all("p")
        ]
        description = "\n".join(paragraphs)
