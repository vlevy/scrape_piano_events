...
class JazzOrgParser(EventParser):
    ...
    def parse_soup_to_event(self, url, soup):
        ...
        dates = [parse_date(e.contents[0].strip().lower()) for e in
                 soup.find_all('a', attrs={'class': 'sqs-block-button-element'})]
        ...
        # Description
        event_description = '\n'.join([str(p.contents[0]) for p in soup.find_all('p', attrs={'class': 'sqsrte-large'})])
